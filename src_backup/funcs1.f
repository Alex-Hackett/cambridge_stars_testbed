...
C Determine if this mesh point is between the required mass coordinates
      YARTSH = 0
      IF ((M.GT.SMASS).AND.(M.LT.FMASS)) THEN
        YARTSH = 1
      END IF 
...
C Inject the Artificial Energy if we are in the right mass region
      IF (YARTSH.EQ.1) THEN
        EN = EN + TENG
        WRITE (*,*) 'Energy Injected in Shell: ', K
      END IF
...
C can't diffuse through H if we don't have any...
         ...
         D4 = D(2)
         D12 = D(3)
         D14 = D(4)C     Final Common for the mass-based artificial energy generation injection
...
C Store thermohaline stuff
         HT(19, K, ISTAR) = SGTH
         HT(20, K, ISTAR) = MU
C Store temp and ln fC     Final Common for the mass-based artificial energy generation injection
      COMMON /ARTENG/ TENG, SMASS, FMASS
         HT(21, K, ISTAR) = AF
         HT(22, K, ISTAR) = AT
